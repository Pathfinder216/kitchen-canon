...
- [x] [06 — Decompose RecipeForm](06-decompose-recipe-form.md) (M-L) — split the 1046-line `RecipeForm.tsx` into `components/recipe-form/` (`useRecipeFormState`, `IngredientsEditor`, `StepsEditor`, `RecipeMetaFields` = BasicInfoFields + NotesAndTaxonomyFields, `CoverPhotoField`, `InlineClassifyPanel`, dialogs in `RefWarningDialogs` + `UnclassifiedWarningDialog`, shared `icons`/`refs`/`styles`); composition file now 210 lines; zero behavior change (11 existing RecipeForm tests pass unmodified) + new `useRecipeFormState` hook tests (seeding precedence, servings parse, payload/orderIndex). Deviations: pure refactor so `['recipe-dietary']` invalidation intentionally NOT added (plan 10); kept `components/RecipeForm.tsx` as the composition file (not a re-export) so the test stays put.
