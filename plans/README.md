...
- [x] [24 — Remaining-percent ingredient refs](24-remaining-percent-refs.md) (M) — bare `{butter}` now means whatever is left after earlier refs (explicit `{butter:NN%}` unchanged, consumption per ref key in step/token order); `computeRemainingPercents` + `priorInstructions` in `utils/resolveIngredientRefs.tsx`, threaded through every caller (detail, print, cook mode + next-step preview + timer labels, shared page, meal-plan preview, .txt/.json export) and the backend schema.org export port; exhausted bare refs render red with a warning title and are flagged by the save-time over-reference dialog; step editor inserts bare `{key}` and its help text documents both forms. No data migration. New 17-test util suite + refs/page/export tests
...
- [x] [28 — Grocery aisle grouping](28-grocery-aisles.md) (M) — `IngredientCatalog.aisle String?` + fixed 13-key vocabulary in `constants/aisles.ts` (served via `GET /api/meta` as `aisles`/`aisleLabels`); all 258 seed entries carry a default aisle. Meal-plan responses attach a read-time `aisle` per grocery item (batched `resolveAisles` in `dietary.service.ts`, user shadow first, a shadow with no aisle falls back to the global's; unresolved → `household-other`). `POST`/`PATCH /api/ingredients` accept an optional `aisle` (omitted = keep; a new shadow of a built-in inherits its aisle). `GroceryList` groups by aisle ("Other" last), copy output has section headers, and a per-item kebab → **Change aisle** modal POSTs a shadow preserving the resolved entry's tags; IngredientsPage editor gains an aisle select. Also fixed `seed.ts` to wipe only **global** catalog rows — it previously deleted every user's private entries on each container start
...
- [x] [32 — Photo/OCR import](32-ocr-import.md) (M; needs 19) — client-side tesseract.js, lazy-loaded into its own chunk; worker/WASM cores/`eng` language data self-hosted under `/ocr/` (no CDN; production CSP gains only `'wasm-unsafe-eval'`); photo downscaled to 1600px, editable OCR text → new `POST /api/import/text` → pre-filled recipe form
