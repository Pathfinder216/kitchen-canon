...
- [x] [25 — Equipment & make-ahead labels](25-equipment-makeahead-labels.md) (S-M) — grew `STANDARD_LABELS` in `prisma/seed.ts` from 4 to 15 global (`userId: null`) `type: 'manual'` labels: equipment (Slow cooker, Instant Pot / pressure cooker, Air fryer, Oven, Stovetop only, No-cook, Grill, Blender / food processor, Stand mixer) + make-ahead/storage (Refrigerate up to 3 days, Night-before prep, alongside existing Make-ahead/Freezable). No schema change and NO label types reintroduced (the old `equipment`/`makeAhead` types stay migrated to `manual`). Added a "plan 37 references these names" stability comment pinning the exact strings. Recipe-form picker + FilterPanel needed no change — both already render manual globals+own alphabetically (the labels API orders by type then name) and multi-select handles the larger set. New `labels-seed.test.ts` (4 tests: full-set seed, run-twice idempotency/count-stable, GET /api/labels returns the globals, user label survives reseed untouched)
