...
describe('Labels', () => {
  it('creates a makeAhead label', async () => {
    ...
    expect(res.body.type).toBe('makeAhead');
  });
  ...
  it('lists labels', async () => {
    await request(app).post('/api/labels').send({ type: 'makeAhead', name: 'freezable' });
    await request(app).post('/api/labels').send({ type: 'equipment', name: 'slow cooker' });
  ...
  it('filters labels by type', async () => {
    ...
    const res = await request(app).get('/api/labels?type=makeAhead');
    expect(res.body).toHaveLength(1);
    expect(res.body[0].name).toBe('freezable');
  });

  it('assigns labels to a recipe', async () => {
    const recipe = await request(app).post('/api/recipes').send({ title: 'Salad' });
    const label = await request(app).post('/api/labels').send({ type: 'makeAhead', name: 'freezable' });
    ...
    expect(res.status).toBe(200);
    const manualLabels = res.body.labels.filter((rl: { isAutoGenerated: boolean }) => !rl.isAutoGenerated);
    expect(manualLabels).toHaveLength(1);
    expect(manualLabels[0].label.name).toBe('freezable');
  });

  it('preserves auto-generated labels when reassigning manual labels', async () => {
    ...
    // Recipe should have auto-generated vegan label from catalog
    const dietaryLabels = recipe.body.labels.filter((rl: { isAutoGenerated: boolean }) => rl.isAutoGenerated);
    expect(dietaryLabels.length).toBeGreaterThan(0);

    // Assign a manual label — auto-generated labels must survive
    const label = await request(app).post('/api/labels').send({ type: 'makeAhead', name: 'freezable' });
    ...
    const autoLabels = updated.body.labels.filter((rl: { isAutoGenerated: boolean }) => rl.isAutoGenerated);
    expect(autoLabels.length).toBeGreaterThan(0);
  });
});
...
describe('Recipe Filtering', () => {
  ...
  it('filters by makeAhead label', async () => {
    await seedRecipes();

    const freeze = await request(app).post('/api/labels').send({ type: 'makeAhead', name: 'freezable' });
    const quick = await request(app).post('/api/labels').send({ type: 'makeAhead', name: 'quick' });
  ...
  it('filters by allergen exclusion (freeFrom)', async () => {
    ...
    expect(r1.body.labels.some((rl: { isAutoGenerated: boolean; label: { name: string } }) =>
      rl.isAutoGenerated && rl.label.name === 'dairy'
    )).toBe(true);
  ...
  it('auto-generated labels are stored on recipe create', async () => {
    ...
    const autoLabels = recipe.body.labels.filter((rl: { isAutoGenerated: boolean }) => rl.isAutoGenerated);
