...
| Meal plans | `GET /api/meal-plans`, `POST /api/meal-plans`, `GET /api/meal-plans/suggestions?recipeIds=&diets=&freeFrom=` (top 6 complementary recipes for the current selection, each with `score`, `reasons[]` and a per-rule `breakdown[]`; pure scorer `scoreCandidates` in `services/suggestions.service.ts`; `[]` below 5 recipes; 404 if a selected id isn't the user's; registered before `/:id`), `GET /api/meal-plans/:id`, `PATCH /api/meal-plans/:id`, `PATCH /api/meal-plans/:id/grocery/:itemId` (toggle purchased), `POST /api/meal-plans/:id/recalculate` (recompute dietary info), `POST /api/meal-plans/:id/remake` (clone), `GET /api/meal-plans/:id/timeline?serveAt=<ISO>` (cooking schedule ending at `serveAt`; see *Cooking timeline* below) |
...
| Ingredients (catalog) | `GET /api/ingredients?q=` (typeahead; appends fuzzy matches when the substring prefilter finds < 5), `GET /api/ingredients/suggest?name=` (top-3 fuzzy "Did you mean …?" matches → `{ id, displayAlias, allergens, diets, aisle, nutrition, score }`), `POST /api/ingredients`, `PATCH /api/ingredients/:id` (both accept optional `aisle` and `nutrition`; omitted = keep, `null` = clear), `DELETE /api/ingredients/:id` |
| Nutrition | `GET /api/nutrition/status` → `{ lookupConfigured }`; `GET /api/nutrition/search?q=` (proxies USDA FoodData Central search, Foundation + SR Legacy, top 5, normalized to the stored per-100 g shape); `GET /api/nutrition/food/:fdcId` (one food incl. `gramsPerUnit` from its portions). Server holds the key (`X-Api-Key` header), 10 s timeout, ≥ 500 ms between upstream calls; 503 when `FDC_API_KEY` is unset, 502 on USDA failure. Only called while classifying — never at recipe render time |
...
| `IngredientCatalog` | Canonical ingredients with `allergens` and `diets` JSON arrays and an optional grocery `aisle` (fixed vocabulary in `constants/aisles.ts`), and optional `nutrition` JSON (`{ per100g: { calories, protein, fat, saturatedFat, carbs, fiber, sugar, sodium }, gramsPerUnit?, source: 'fdc'\|'manual'\|'copied', fdcId?, fdcDescription? }` — kcal/g/mg-sodium, validated by `schemas/nutrition.schema.ts`); powers dietary auto-labeling, typeahead, grocery aisle grouping, and nutrition. Nullable `userId` (null = global seed of ~258 entries; non-null = user's private entry, preferred on lookup — also how a user reassigns an aisle). |
...
| `FDC_API_KEY` | unset (lookup disabled) | USDA FoodData Central key for the nutrition lookup (free at https://fdc.nal.usda.gov/api-key-signup); when unset the lookup UI is hidden and nutrition can still be entered manually |
